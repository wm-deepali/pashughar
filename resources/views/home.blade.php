...
@section('content')
    <style>
        ...
        .widget-card {
            ...
            align-items: center;
            color: #fff;
            /* Text color for better contrast with gradients */
        }
    ...
    <div class="container-fluid">
        <div class="dashboard-main-card pt-3">
            ...
            <div class="widget-card widget-5">
                <div class="widget-content">
                    <h3>Expired Ads</h3>
                    <p>Total: &nbsp;{{$countExpiredAds}}</p>
                </div>
                <i class="fas fa-hourglass-end widget-icon"></i>
            </div>
                        ...
                        <div class="card-body">
                            <table class="table table-bordered table-hover table-striped">
                                ...
                                <tbody>
                                    ...
                                    @if(isset($expiredAds) && count($expiredAds) > 0)
                                        @foreach($expiredAds as $ad)
                                            <tr>

                                                <td class="text-danger">
                                                    {{ $ad->expire_at ? date('d/m/Y', strtotime($ad->expire_at)) : "" }}</td>
                                                <td>{{$ad->title ?? "" }}</td>
                                                <td>{{$ad->category->name ?? "" }}</td>
                                                <td>{{$ad->author_name ?? "" }} <br />{{$ad->author_email ?? "" }}</td>
                                            </tr>
                                    ...
                                    @else
                                        <tr>
                                            <td colspan="4" class="text-center text-muted">No expired ads found.</td>
                                        </tr>
                                    @endif
                                </tbody>
                            </table>
                                    ...
                                    @if(isset($history) && count($history) > 0)
                                        @foreach($history as $hist)
                                            <tr>
                                                ...
                                                <td>{{$hist->customers->full_name ?? "" }} <br />{{$hist->customers->email ?? "" }}
                                                </td>
