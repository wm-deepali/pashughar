                        ...
                        <div class="col-md-6 col-lg-4" style="margin-top: 20px;">
                            ...
                            <div class="price-card">
                                @if($subscription->offer_price == 0)
                                    @if(!$subscription->is_free_available)
                                        <div id="overlay">
                                            <div id="text">You have already used the Free Ad posting Limit</div>
                                        </div>
                                    @elseif(Auth::guard('member')->user()->active_subscription_id == $subscription->id)
                                        <div id="overlay">
                                            <div id="text">Current Active Plan</div>
                                        </div>
                                    @endif
                                @endif
                                <div class="price-head">
                                    ...
                                    @if($subscription->discount > 0 && $subscription->offer_price > 0)
                                        <h3><span style="font-size:20px">₹&nbsp;</span>{{$subscription->offer_price}} <strike
                                                class="strike">{{$subscription->mrp}}</strike> <span
                                                class="discount">{{$subscription->discount}}%</span></h3>
                                    @else
                                            ...
                                            @if($subscription->offer_price == 0)
                                                @if(Auth::guard('member')->user()->is_buy_free_subscription == 0)
                                                    @if(Auth::guard('member')->user()->state != '')
                                                        <button class='btn btn-inline pay_now' name="pay_now" id="pay_now"
                                                            subscription_id="{{$subscription->id}}"><i class="fas fa-sign-in-alt"></i><span>Free
                                                                Subscription</span></button>
                                                    @else
                                                        <a class='btn btn-inline' href="{{route('user.settings')}}"><i
                                                                class="fas fa-sign-in-alt"></i><span>Free Subscription</span></a>
                                                    @endif
                                                @else
                                                    <a href="javascript:void(0);" class='btn btn-inline'
                                                        onclick="return confirm('Your buy free subscription limit end');"><i
                                                            class="fas fa-sign-in-alt"></i><span>Free Subscription</span></a>
                                                @endif
                                            ...
                                            @else
                                                @if(Auth::guard('member')->user()->no_of_ads == 0 || Auth::guard('member')->user()->expiry_date < date('Y-m-d'))
                                                    <a class='btn btn-inline'
                                                        href="{{route('user.checkout', Crypt::encrypt($subscription->id))}}">
                                                ...
                                                @else
                                                    ...
                                                    <a href="javascript:void(0);" class='btn btn-inline'
                                                        onclick="return confirm('Ads are already available to publish in your active subscription. Please use all the ads in the bucket first.');"><i
                                                            class="fas fa-sign-in-alt"></i><span>Buy Subscription</span></a>
                                                @endif
                                            @endif
                                        @else
                                            <a class='btn btn-inline' href="{{route('user.login')}}"><i
                                                    class="fas fa-sign-in-alt"></i><span>Buy Subscription</span></a>
                                        @endif
                                ...
                                <ul class="price-list" style="padding-left:0px;">
                                    ...
                                    <li>
                                        <i class="fas fa-check"></i>
                                        <p>{{$subscription->subscription_validity}} &nbsp;Days Validity
                                        </p>
