...
class SubscriptionController extends Controller
{
    ...
    public function update(Request $request, string $id)
    {
        ...
        // If admin checked "update users"
        if ($request->has('update_users') && $request->update_users == 1) {

            // Get all members with this subscription as their active one
            $users = Member::where('active_subscription_id', $subscription->id)->get();

            foreach ($users as $user) {

                // Update SubscriptionOrder (user's active subscription)
                $order = SubscriptionOrder::where('user_id', $user->id)
                    ->where('subscription_id', $subscription->id)
                    ->first();

                if ($order) {
                    $order->category_id = !empty($request->category_id) ? implode(',', $request->category_id) : null;
                    $order->save();
                }

                // Update all subscription_history records for this subscription
